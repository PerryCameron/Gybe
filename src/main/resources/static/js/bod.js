...
function fillPositions() {
  officers = boardOfDirectors.filter((member) => member.officer === true);
  chairs = boardOfDirectors.filter((member) => member.chair === true);
  assistants = boardOfDirectors.filter((member) => member.assistantChar === true);
  board1 = boardOfDirectors.filter((member) => member.boardYear === year);
  board2 = boardOfDirectors.filter((member) => member.boardYear === year + 1);
  board3 = boardOfDirectors.filter((member) => member.boardYear === year + 2);
}
...
function createBoardTable() {
  ...
  const headerRow = document.createElement("tr");
  let headers = [year, year + 1, year + 2]; // Column headers based on years
